...
> \`nowplaying\`, \`queue\`, \`volume\``)
            .setFooter("2020 Weird.exe during covid-19");
    ...
    if (command === "play" || command === "p") {
        ...
        if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
            ...
            return msg.channel.send(`<:thumbsup:591629527571234819>  **|**  Playlist: **\`${playlist.title}\`** has been added to the queue!`);
        } else {
            try {
...
bot.on("ready", () => {
  ...
  bot.user.setActivity("minecraft music", { type: "LISTENING" });
});
